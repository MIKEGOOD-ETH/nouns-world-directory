...
const Pill = ({ children, selected, onClick }) => (
  <button
    type="button"
    onClick={onClick}
    className={\`select-none whitespace-nowrap rounded-2xl border px-3 py-1 text-sm transition \${selected
    ...
    <div className="mx-auto max-w-6xl px-4 pb-24">
      ...
      {loading ? (
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((r) => (
            <article key={r.key} className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm hover:shadow-md transition">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {r.categories.map((c) => (
                  <span key={c} className="rounded-full border bg-neutral-50 px-2 py-0.5 text-xs text-neutral-700">
                    {c}
                  </span>
                ))}
              </div>
